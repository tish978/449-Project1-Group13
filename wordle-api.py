...
async def login_test():
    ...
    row = await db.execute(query=query, values={"id": entered_id, "pass": entered_pass})
    if row:
        return jsonify({"authenticated": "true"})
    else:
        abort(404)
...
async def login_user(id, password):
    db = await _get_db()
    data = await request.get_json()
    book = await db.execute("SELECT * FROM users WHERE user_id = :user_id AND password = :password", values={"user_id": id, "password": password})
    if book:
        return jsonify({"authenticated": "true"})
    else:
        abort(404)        
...
async def get_games_in_progress():
    ...
    connection = sqlite3.connect("////home/student/Documents/449-wordle/wordle-DB")
    print("DB connected")
    query = "SELECT * FROM games WHERE user_id = ? AND won = ?"
    initCursor = connection.execute(query, [entered_id, 0])
...
async def get_game_state():
    ...
    initCursor = connection.execute(query, [entered_id])
    for game in initCursor:
    	guessesMade = game[3]
    	won = game[2]
    	if won == 1:
